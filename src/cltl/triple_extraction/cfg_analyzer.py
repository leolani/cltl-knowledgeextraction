...
class CFGAnalyzer(Analyzer):
    ...
    def analyze(self, chat):
        ...
        CFGAnalyzer.PARSER.parse(self._utterance)
    ...
    def fix_triple_details_subject_object(self, triple, utterance_info):
        ...
        if not triple['object'] == None:
            if len(triple['object'].split('-')) > 1:  # multi-word object, consisting of a phrase
                logger.debug('multi-word object, consisting of a phrase', triple['object'])
                triple = self.analyze_multiword_complement(triple)
            elif len(triple['object'].split('-')) == 1:
                triple = self.analyze_one_word_complement(triple)
            logger.debug('after object analysis: {}'.format(triple))
    ...
    def get_kinship(self, triple, utterance_info):
        ...
        if triple['predicate'] == "be" or triple['predicate'] == "is":
            ...
            if predicate and kinship_word:
                ...
                logger.debug('o-kinship', triple)
        elif triple['predicate'] == "is-named" or triple['predicate'] == "is-called" or triple[
            ...
            if predicate and kinship_word:
                ...
                if first == 'my':
                    ...
                    triple['object'] = first
                logger.debug('s-kinship-called', triple)
    ...
    def get_activity(self, triple, utterance_info):
        ...
        if (lemmatize(triple['predicate'], 'v') == "be" or
            ...
            if predicate and activity_word:
                ...
                logger.debug('o-activity', triple)
        return triple, utterance_info, activity_word

    def get_condition(self, triple, utterance_info):
        condition_word = ""
        if triple['predicate'] == "be" or triple['predicate'] == "am" or triple['predicate'] == "is" or triple[
            'predicate'] == "was":
            ...
            if predicate and condition_word:
                ...
                logger.debug('o-condition', triple)
            ...
            if predicate and condition_word:
                ...
                triple['object'] = condition_word
                logger.debug('o-condition', triple)
            logger.debug('predicative reading triple', triple)
        return triple, utterance_info, condition_word

    def get_location(self, triple, utterance_info):
        container_word = ""
        if lemmatize(triple['predicate'], 'v') == "be" and (
                triple['object'].startswith("in-") or triple['object'].startswith("on-")):
            ...
            if container_word:
                ...
                logger.debug('container_word', triple)
    ...
    def get_profession(self, triple, utterance_info):
        ...
        if lemmatize(triple['predicate'], 'v') == "be":
            ...
            if profession_word:
                ...
                logger.debug('container_word', triple)
        elif lemmatize(triple['predicate'], 'v') == "work" and triple['object'].startswith("as-a-"):
            triple['predicate'] = "work-as"
            triple['object'] = triple['object'][5:]
            logger.debug('predicative reading triple', triple)
        elif lemmatize(triple['predicate'], 'v') == "work" and triple['object'].startswith("as-"):
            triple['predicate'] = "work-as"
            triple['object'] = triple['object'][3:]
            logger.debug('predicative reading triple', triple)
        elif lemmatize(triple['predicate'], 'v') == "work" and triple['object'].startswith("as a "):
            triple['predicate'] = "work-as"
            triple['object'] = triple['object'][5:]
    ...
    def analyze_np(self, triple):
        ...
        if len(triple['subject'].split('-')) > 1:
            ...
            triple['subject'] = fix_pronouns(triple['subject'], self.utterance.utterance_speaker, self.utterance._chat_speaker,
    ...
    def analyze_one_word_subject(self, triple):
        ...
        if lexicon_lookup(triple['subject'].lower()) and 'person' in lexicon_lookup(triple['subject'].lower()):
            if triple['predicate'] == 'be':
                subject = fix_pronouns(triple['subject'], self.utterance.utterance_speaker, self.utterance._chat_speaker, self.utterance._chat_agent)
                print('subject after pronoun fix', subject)
                pred = ''
            ...
            triple['object'] = ''
        elif get_pos_in_tree(CFGAnalyzer.PARSER.structure_tree, triple['subject']).startswith('PRP'):
    ...
    def analyze_one_word_complement(self, triple):
        ...
        if lexicon_lookup(triple['object']) and 'person' in lexicon_lookup(triple['object']):
            if triple['predicate'] == 'be':
                subject = fix_pronouns(triple['object'].lower(), self.utterance._chat_speaker,
                                       self.utterance._chat_agent)
                pred = ''
                for el in triple['subject'].split('-')[1:]:
                    pred += el + '-'
            ...
            triple['object'] = ''
        elif get_pos_in_tree(CFGAnalyzer.PARSER.structure_tree, triple['object']).startswith('PRP'):
            triple['object'] = fix_pronouns(triple['object'].lower(), self.utterance.utterance_speaker, self.utterance._chat_speaker,
                                            self.utterance._chat_agent)
...
class GeneralStatementAnalyzer(StatementAnalyzer):
    ...
    def analyze(self, utterance):
        ...
        if preempted: logger.debug('KINSHIP TRIPLE', preempted, triple)
