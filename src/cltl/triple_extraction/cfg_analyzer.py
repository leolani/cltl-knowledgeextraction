...
class CFGAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        if not CFGAnalyzer.NER:
            CFGAnalyzer.NER = NER('english.muc.7class.distsim.crf.ser')
    ...
    def analyze(self, utterance):
        ...
        if not self.utterance.parser.forest:
            self._log.warning("Couldn't parse input")

        else:
            for tree in self.utterance.parser.forest:
    ...
    def fix_predicate(self, predicate):
        ...
        if '-' not in predicate:
            ...
            if get_pos_in_tree(self.utterance.parser.structure_tree, predicate) in ['IN', 'TO']:
    ...
    def analyze_vp(self, triple, utterance_info):
        ...
        if len(triple['predicate'].split('-')) == 1:
            ...
            label = get_pos_in_tree(self.utterance.parser.structure_tree, triple['predicate'])
        ...
        for el in triple['predicate'].split('-'):
            label = get_pos_in_tree(self.utterance.parser.structure_tree, el)
            # negation
            if label == 'RB':
            ...
            # verbs that carry sentiment or certainty are considered followed by their object
                ...
                pred += '-' + lemmatize(el, 'v')
                for elem in triple['predicate'].split('-')[ind + 1:]:
                    label = get_pos_in_tree(self.utterance.parser.structure_tree, elem)
                    if label in ['TO', 'IN']:
                        pred += '-' + elem
                    else:
                        triple['object'] = elem + '-' + triple['object']
    ...
    def analyze_one_word_complement(self, triple):
        ...
        elif get_pos_in_tree(self.utterance.parser.structure_tree, triple['object']).startswith('V') \
                and get_pos_in_tree(self.utterance.parser.structure_tree, triple['predicate']) == 'MD':
    ...
    def analyze_one_word_subject(self, triple):
        ...
        if lexicon_lookup(triple['subject']) and 'person' in lexicon_lookup(triple['subject']):
            if triple['predicate'] == 'be':
                ...
                triple['object'] = fix_pronouns(triple['object'].lower(), self.utterance.chat_speaker)
        elif get_pos_in_tree(self.utterance.parser.structure_tree, triple['subject']).startswith('V') \
                and get_pos_in_tree(self.utterance.parser.structure_tree, triple['predicate']) == 'MD':
    ...
    def analyze_multiword_subject(self, triple):
        ...
        if get_pos_in_tree(self.utterance.parser.structure_tree, first_word) in ['TO', 'IN']:
    ...
    def get_types_in_triple(self, triple):
        ...
        for el in triple:
            ...
            triple[el]['type'] = get_triple_element_type(text, self.utterance.parser.structure_tree)
                    ...
                    if triple[el]['type'][typ] in [None, '']:
                        ...
                        if entry is None:
                            if typ.lower() in ['leolani']:
                                ...
                                node = get_pos_in_tree(self.utterance.parser.structure_tree, typ)
                                if node in ['IN', 'TO']:
...
class GeneralStatementAnalyzer(StatementAnalyzer):
    ...
    def analyze(self, utterance):
        ...
        if entry and 'certainty' in entry:
            if self.utterance.parser.constituents[2]['label'] == 'S':
        ...
        #### processing the subject in the triple
        ...
        # Extract perspective
        perspective = self.extract_perspective(triple['predicate'], utterance_info)
    ...
    def analyze_certainty_statement(self, triple):
        ...
        """
        index = 0
        for subtree in self.utterance.parser.constituents[2]['structure']:
            text = ''
    ...
    def initialize_triple(self):
        ...
        """
        triple = {'subject': self.utterance.parser.constituents[0]['raw'],
                  'predicate': self.utterance.parser.constituents[1]['raw'],
                  'object': self.utterance.parser.constituents[2]['raw']}
        return triple
    ...
    def analyze_multiword_complement(self, triple):
        ...
        if get_pos_in_tree(self.utterance.parser.structure_tree, first_word).startswith('V'):
            if get_pos_in_tree(self.utterance.parser.structure_tree, triple['predicate']) == 'MD':
...
class WhQuestionAnalyzer(QuestionAnalyzer):
    ...
    def analyze(self, utterance):
        ...
        utterance_info = {'neg': False,
                          'wh_word': lexicon_lookup(self.utterance.parser.constituents[0]['raw'].lower())}
        ...
        #### processing the object in the triple
    ...
    def initialize_triple(self):
        ...
        constituents = self.utterance.parser.constituents
        if len(constituents) == 3:
            label = get_pos_in_tree(self.utterance.parser.structure_tree, constituents[2]['raw'])
            ...
            label = get_pos_in_tree(self.utterance.parser.structure_tree, constituents[1]['raw'])
...
class VerbQuestionAnalyzer(QuestionAnalyzer):
    ...
    def analyze_multiword_complement(self, triple):
        structure_tree = self.utterance.parser.structure_tree
        first_word = triple['object'].split('-')[0]
        if lexicon_lookup(first_word) and 'person' in lexicon_lookup(first_word):
            triple = self.analyze_possessive(triple, 'object')
